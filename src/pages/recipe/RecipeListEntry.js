            ...
            <>
                <div className={`recipe-list-instance mb-2 ps-4 pe-4 ${!this.props.hideArrow ? "pt-3" : ""} pb-2 app-card ${recipe.type}`} onClick={this.props.onClick}>
                    <Row onClick={this.toggleExpansion} className={`cursor-pointer ${!this.props.hideArrow ? "header-row" : ""}`}>
                        ...
                        {this.props.edit || this.props.editRecipe ?
                            <Col xs={2} md={1} className={`d-flex flex-row justify-content-end`}>
                                <div className={`icon-container me-2 ${this.props.editRecipe ? "" : "d-none"}`} onClick={() => this.setState({modalShow: true})}>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                         className="bi bi-pencil" viewBox="0 0 16 16">
                                        <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/>
                                    </svg>
                                </div>
                                <div className={`icon-container ${this.props.edit ? "": "d-none"}`} onClick={event => {
                    ...
                    {!this.props.hideArrow ?
                        <Row onClick={this.toggleExpansion} className="cursor-pointer">
                            {this.state.expanded ?
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                ...
                <Modal show={this.state.showDeleteConfirm} onHide={() => this.setState({showDeleteConfirm: false})}>
                    ...
                    <Modal.Body>Are you sure you want to delete the recipe {this.props.recipe.name}</Modal.Body>
                    <Modal.Footer>
